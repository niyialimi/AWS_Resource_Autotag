...
def lambda_handler(event, context):
    ...
    if event_name == 'CreateTopic':
        ...
        try:
            user_added_tags = sns_client.list_tags_for_resource(ResourceArn=sns_topic_arn)['Tags']
        ...
        try:
            user_added_tags = s3_client.get_bucket_tagging(Bucket=s3_bucket_name)['TagSet']
            tag_set['Tags'] += user_added_tags
        except s3_client.exceptions.NoSuchTagSet:
            pass  # No existing tags to add
        finally:
            s3_client.put_bucket_tagging(Bucket=s3_bucket_name, Tagging=tag_set)

    elif event_name == 'RunInstances':
        ec2_instance_id = detail['responseElements']['instancesSet']['items'][0]['instanceId']
        try:
            user_added_tags = ec2_client.describe_tags(Filters=[{'Name': 'resource-id', 'Values': [ec2_instance_id]}])['Tags']
            tag_set['Tags'] += user_added_tags
        ...
        try:
            ...
            iam_client.tag_role(RoleName=iam_role_name, Tags=tag_set['Tags'])

    elif event_name == 'CreateDBInstance':
        rds_instance_id = detail['responseElements']['dBInstanceIdentifier']
        try:
            user_added_tags = rds_client.list_tags_for_resource(ResourceName=rds_instance_id)['TagList']
            tag_set['Tags'] += user_added_tags
        except rds_client.exceptions.DBInstanceNotFoundFault:
            pass  # No existing tags to add
        finally:
            rds_client.add_tags_to_resource(ResourceName=rds_instance_id, Tags=tag_set['Tags'])

    elif event_name == 'CreateFunction':
        lambda_function_name = detail['responseElements']['functionName']
        try:
            user_added_tags = lambda_client.list_tags(Resource=lambda_function_name)['Tags']
            tag_set['Tags'] += user_added_tags
        except lambda_client.exceptions.ResourceNotFoundException:
            pass  # No existing tags to add
        finally:
            lambda_client.tag_resource(Resource=lambda_function_name, Tags=tag_set['Tags'])

    elif event_name == 'CreateLogGroup':
        log_group_name = detail['requestParameters']['logGroupName']
        try:
            user_added_tags = cloudwatchlogs_client.list_tags_log_group(logGroupName=log_group_name)['tags']
            tag_set['Tags'] += user_added_tags
        except cloudwatchlogs_client.exceptions.ResourceNotFoundException:
            pass  # No existing tags to add
        finally:
            cloudwatchlogs_client.tag_log_group(logGroupName=log_group_name, tags=tag_set['Tags'])
